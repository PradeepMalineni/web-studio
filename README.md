...
A Claude skill that transforms and improves websites from scratch  -  with AI-generated images, competitive research, and production-ready code.

## What It Does

Invoke `/launch-website <URL>` and the skill will:
...
4. **Build** the site with semantic HTML5, CSS custom properties, and vanilla JS
5. **Research** 5 competitors and analyze their design/UX patterns
6. **Improve** the transformed site using competitive insights
7. **Deliver** production-ready files + full README

## Usage

```
/launch-website https://example.com
/launch-website ./local-export.html ./my-output-dir
```
...
- **Transform, don't copy**  -  transform and improve, don't clone
- **Preserve brand identity**  -  keep colors, name, logo style intact
- **No placeholder text**  -  all content must be real or improved
- **imgbb for permanence**  -  Kie AI URLs expire; always upload to imgbb
- **Minimal deps**  -  CDN over npm unless React is genuinely needed

## Installation

Copy the `web-studio/` folder into your Claude skills directory, or install the `.skill` package if provided.
